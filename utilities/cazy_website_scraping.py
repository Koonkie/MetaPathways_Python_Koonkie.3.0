...
rx_kingdom = re.compile(r'class="choix"><a href="(http://www\.cazy\.org/\w+_(archaea|bacteria|eukaryota|viruses|characterized|structure)\.html)"')
...
rx_EC = re.compile(r'(\d+[.]\d+[.]\d+[.]\d+)')
rx_species = re.compile(r'target="ncbitaxid">(.*)</a>')
rx_accession = re.compile(r'=protein&amp;val=(.*)" target="')
      ...
      #iterate the url here
                for taxonurl_address in taxonurl:
                    ...
                    for row in tabulka:
                        ...
                        #print "1", str(cols[1]).strip()
                        #print "2", str(cols[2]).strip()
                        #print "3", str(cols[3]).strip()

                        ecres = rx_EC.search(str(cols[1]))
                        ec = ''
                        species=''
                        accession=''
                        try:
                           ...
                           speciesres = rx_species.search(str(cols[2]))
                           if speciesres:
                            species=speciesres.group(1)


                           accres = rx_accession.search(str(cols[3]))
                           if accres:
                              accession= accres.group(1)
                        except:
                           pass

                        if accession!='':
                           fprintf(outfile, "%s\t%s\t%s\t%s\n", accession, cazy_name,  ec, species)
